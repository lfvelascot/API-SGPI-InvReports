...
public class ProgramaRService {
	...
	public List<ProgramaR> getProgramasFacultad(List<Programa> programas) {
		...
		for (Programa p : programas) {
			salida.add((p.getDirector() != null)
					? new ProgramaR(p.getNombre(), p.getDirector().getNombreCompleto(), p.getDirector().getCorreoEst(),
							p.getDirector().getTelefono())
					: new ProgramaR(p.getNombre(), "Sin registro", "Sin registro", "Sin registro"));
		}
		return salida;
	}
